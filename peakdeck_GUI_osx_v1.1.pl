				...
				if ($option eq 'PeakCall')
					{
						...
						die "warning: sam file chromosomes for inputfile (\'$samForReport\') appear disordered.\n\n"	if $samTestResults[3] eq 'BAD';
				...
				if ($option eq 'DensityAnalyser')
					{
						...
						die "warning: sam file chromosomes for inputfile (\'$samForDensity\') appear disordered.\n\n"	if $samTestResults[3] eq 'BAD';
		...
		if ($closedAndOpen eq "NO")
			{
				...
				my $hashCount = keys %randomChromHash;
...
'=head1 NAME
	...
	PeaKDEck v1.1 - a perl/Tk kernel density estimator based peak caller for I<DNaseI-seq> data. 
...
=head2 Peak calling 
	...
	file. Sam files must be grouped by chromosomes, and ordered by read position (see Numerical sorting 
